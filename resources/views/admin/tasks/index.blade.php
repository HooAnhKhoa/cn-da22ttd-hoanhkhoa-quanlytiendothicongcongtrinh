                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                {{-- <div class="text-sm text-gray-500">Ngân sách: {{ number_format($task->total_budget) }} VNĐ</div> --}}
                            </div>
                        </div>
                    </td>
                    {{-- task site name --}}
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">{{ $task->site->site_name }}</div>
                    </td>
                    {{-- start date --}}
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            ...
                            <span class="text-sm text-gray-600">{{ $task->progress_percent }}%</span>
                        </div>
                    </td>
                    {{-- status --}}
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full 
                            ...
                            @if($task->status == 'in_progress') Đang thi công
                            @elseif($task->status == 'completed') Hoàn thành
                            @elseif($task->status == 'on_hold') Tạm dừng
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function applyFilters() {
        ...
        window.location.href = '{{ route('admin.tasks.index') }}?' + params.toString(); // Sửa route từ projects.index thành tasks.index
    }
    ...
    resetButton.addEventListener('click', function() {
        window.location.href = '{{ route('admin.tasks.index') }}'; // Sửa route từ projects.index thành tasks.index
    });
